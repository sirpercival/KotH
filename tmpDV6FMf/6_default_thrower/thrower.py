...
class Bot(object):
    ...
    def read_board(self):
        ...
        for d, (dx, dy) in DIRECTIONS.items():
            ...
            '''if ((y - dy) < 1 or (y - dy) > len(board) or 
                (x + dx) < 1 or (x + dx) > len(board[y])):
                self.neighbors[d] = ''
            else:
                self.neighbors[d] = board[y-dy][x+dx]'''
    ...
    def act(self, board):
        ...
        self.read_board()
        action = self.action()
        pre = self.previous_action.split()
        cur = self.current_action.split()
        if (pre[0] == 'throw' and self.neighbors[pre[1]] in THROW and not cur[0] == 'throw'):
            print('failed at {:d}'.format(self.turn), file=sys.stderr)
        self.save()
        print(action)
